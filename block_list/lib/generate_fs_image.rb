class HdfsImage
  ...
  def output_inode_dir(id, name, quota="-1")
    @fh.puts "<inode><id>#{id}</id><type>DIRECTORY</type><name>#{name}</name><mtime>1554197634733</mtime><permission>sodonnell:supergroup:0755</permission><nsquota>#{quota}</nsquota><dsquota>-1</dsquota></inode>"
  end

  def output_inode_file(id, name, block_id, genstamp, rep_factor, size)
    @fh.puts "<inode><id>#{id}</id><type>FILE</type><name>#{name}</name><replication>#{rep_factor}</replication><mtime>1554285467270</mtime><atime>1554285466577</atime><preferredBlockSize>134217728</preferredBlockSize><permission>sodonnell:supergroup:0644</permission><blocks><block><id>#{block_id}</id><genstamp>#{genstamp}</genstamp><numBytes>#{size}</numBytes></block>
